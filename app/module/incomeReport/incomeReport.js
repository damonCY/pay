...
app.registerController('incomeReportCtrl', ['$rootScope', '$scope', '$http', '$modal', '$state', '$stateParams', '$sce', 'dateFilter',
    function($rootScope, $scope, $http, $modal, $state, $stateParams, $sce, dateFilter) {
    $scope.title = '收入报表';
    $scope.titleIcon = '';

    var irYear = dateFilter($rootScope.startTime, 'yyyy');
    var yearsOptions = [];
    for(var year = 2010; year <=irYear; year++ ) {
    ...
    // init params
    $scope.queryParams = {
        irYear: dateFilter($rootScope.startTime, 'yyyy')
    };
    ...
    $scope.search = function () {
    	$state.go('incomeReport', $scope.queryParams, {
            reload: true
        });
    };
    ...
    $scope.export = function ($event) {
        ...
        $http.post('/incomeReport/export', $scope.queryParams, {
