...
app.registerDirective('miSelect',  [function(){
    return {
        restrict: 'E',
        replace: true,
            template: __inline('miSelect.html'),
            scope: {
            theme: '@',
            options: '=',
            selected: '=',
    ...
    function setDisplayName($scope, iElement) {
        if ($scope.selected) {
            ...
            if ($scope.value){
                ...
                $scope.value = $scope.options[0].value || undefined;
            }
        }
    }
    ...
    function setSelected($scope, iAttrs, option) {
        ...
        // 设置选项的常用value值
        if (iAttrs.value) {
            $scope.value = (typeof option.value != undefined) ?  option.value : undefined;
        }
    }
}]);
